// Nodo: Conversión a Chart (Actualizado para Mediciones_Sensores - Humedad)
// Entrada esperada en msg:
//   payload: array de filas [{ periodo, Humedad }, …]
...
const dataMap = {};
rows.forEach(row => {
    if (row && row.periodo != null && row.Humedad !== undefined) {
        dataMap[row.periodo] = {
            humedad: Number(row.Humedad) || 0
        };
    } else {
        node.warn(`Fila ignorada por periodo inválido o falta Humedad: ${JSON.stringify(row)}`);
    }
});
...
fullPeriods.forEach(period => {
    ...
    const entry = dataMap[period] || { humedad: 0 };
    chart.data[0].push(entry.humedad);
});
