// Nodo: Conversión a Chart (Actualizado para Mediciones_Sensores - Piranometro)
// Entrada esperada en msg:
//   payload: array de filas [{ periodo, Piranometro }, …]
...
const dataMap = {};
rows.forEach(row => {
    if (row && row.periodo != null && row.Piranometro !== undefined) {
        dataMap[row.periodo] = {
            piranometro: Number(row.Piranometro) || 0
        };
    } else {
        node.warn(`Fila ignorada por periodo inválido o falta Piranometro: ${JSON.stringify(row)}`);
    }
});
...
fullPeriods.forEach(period => {
    ...
    const entry = dataMap[period] || { piranometro: 0 };
    chart.data[0].push(entry.piranometro);
});
