// Nodo: Conversión a Chart (Actualizado para Mediciones_Sensores - Direccion_Viento)
// Entrada esperada en msg:
//   payload: array de filas [{ periodo, Direccion_Viento }, …]
...
const dataMap = {};
rows.forEach(row => {
    if (row && row.periodo != null && row.Direccion_Viento !== undefined) {
        dataMap[row.periodo] = {
            direccion_viento: Number(row.Direccion_Viento) || 0
        };
    } else {
        node.warn(`Fila ignorada por periodo inválido o falta Direccion_Viento: ${JSON.stringify(row)}`);
    }
});
...
fullPeriods.forEach(period => {
    ...
    const entry = dataMap[period] || { direccion_viento: 0 };
    chart.data[0].push(entry.direccion_viento);
});
